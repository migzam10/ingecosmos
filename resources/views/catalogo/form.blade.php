...
@section('title', isset($catalogo) ? 'Editar Ítem' : 'Nuevo Ítem')
@section('page_title', isset($catalogo) ? 'Editar Ítem del Catálogo' : 'Nuevo Ítem de Catálogo')
@section('breadcrumb', 'Catálogo MO')
...
<form method="POST"
      action="{{ isset($catalogo) ? route('catalogo.update', $catalogo) : route('catalogo.store') }}">
    @csrf
    @if(isset($catalogo)) @method('PUT') @endif

    <div class="card">
        <div class="card-header">
            <h3 class="card-title">
                {{ isset($catalogo) ? 'Editando: ' . $catalogo->descripcion : 'Nuevo ítem de Mano de Obra' }}
            </h3>
        </div>
        <div class="card-body">
                    ...
                    <div class="col-12 col-md-4">
                        <label class="form-check form-check-card">
                            <input class="form-check-input" type="radio" name="nivel"
                                   value="{{ $n }}" id="nivel{{ $n }}"
                                   {{ old('nivel', $catalogo->nivel ?? 1) == $n ? 'checked' : '' }}
            ...
            <div id="campo-marca" class="mb-3"
                 style="{{ old('nivel', $catalogo->nivel ?? 1) == 1 ? 'display:none' : '' }}">
                ...
                <select name="id_marca" id="select-marca-cat" class="form-select"
                    ...
                    <option value="{{ $marca->id }}"
                        {{ old('id_marca', $catalogo->id_marca ?? '') == $marca->id ? 'selected' : '' }}>
                        {{ $marca->nombre }}
                    </option>
                    @endforeach
                </select>
            ...
            <div id="campo-modelo" class="mb-3"
                 style="{{ old('nivel', $catalogo->nivel ?? 1) != 3 ? 'display:none' : '' }}">
                ...
                <select name="id_modelo" id="select-modelo-cat" class="form-select">
                    ...
                    <option value="{{ $modelo->id }}"
                        {{ old('id_modelo', $catalogo->id_modelo ?? '') == $modelo->id ? 'selected' : '' }}>
                        {{ $modelo->nombre }}
                    </option>
                    @endforeach
                    @endisset
            ...
            <div class="mb-3">
                ...
                <input type="text" name="descripcion" class="form-control @error('descripcion') is-invalid @enderror"
                       value="{{ old('descripcion', $catalogo->descripcion ?? '') }}"
                       placeholder="Ej: Desmonte y montaje puerta delantera"
            ...
            <div class="mb-0">
                <label class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" name="activo" value="1"
                           {{ old('activo', $catalogo->activo ?? true) ? 'checked' : '' }}>
                    <span class="form-check-label">Ítem activo en el catálogo</span>
                </label>
            </div>
        ...
        <div class="card-footer d-flex gap-2">
            <button type="submit" class="btn btn-primary">
                {{ isset($catalogo) ? 'Guardar Cambios' : 'Agregar al Catálogo' }}
            </button>
