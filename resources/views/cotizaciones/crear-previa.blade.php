...
const ITEMS_RTO_EDIT = @json($cotizacion->itemsRepuesto->map(fn($i) => ['descripcion' => $i->descripcion, 'unidades' => (float)$i->unidades, 'precio_unitario' => (float)$i->precio_unitario, 'precio_total' => (float)$i->precio_total, 'id_catalogo_repuesto' => $i->id_catalogo_repuesto]));
...
document.getElementById('inp-iva-pct').addEventListener('input', function () {
    const subtotal = sumCol('body-mo','inp-precio-mo') + sumCol('body-rto','inp-total-rto');
